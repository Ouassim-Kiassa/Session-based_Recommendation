...
class SessionDataset:
    ...
    def __init__(self, data, sep='\t', session_key='session_id', item_key='item_id', time_key='date', n_samples=-1, itemmap=None, time_sort=False):
        """
        Args:
...
if __name__ == '__main__':
    ...
    args.train_data = pd.read_csv(args.train_path, dtype={'item_id': np.int64})
    args.dev_data   = pd.read_csv(args.dev_path, dtype={'item_id': np.int64})
    args.test_data  = pd.read_csv(args.test_path, dtype={'item_id': np.int64})
    args.train_n_items = len(args.train_data['item_id'].unique()) + 1

    args.train_samples_qty = len(args.train_data['session_id'].unique()) + 1
    args.test_samples_qty = len(args.test_data['session_id'].unique()) + 1
